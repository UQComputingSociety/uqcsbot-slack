...
@bot.on_command('yt')
async def handle_yt(command: Command):
    '''
    ...
    if command.has_arg():
        try:
            ...
            await yt_normal(cmd, command.channel)
        except HttpError as e:
            # Googleapiclient should handle http errors
            print('An HTTP error %d occurred:\n%s' % (e.resp.status, e.content))
    else:
        bot.post_message(command.channel, "You can't look for nothing.")
        
async def yt_normal(search_query: str, channel):
