...
def format_files(files: List[dict]) -> List[str]:
    """
    Takes the list of file dictionaries found and formats
    them into Slack-appropriate message strings.

    :param files: a list of dictionaries where each dictionary
    represents a file in the Google Drive folder.
...
def get_all_files(folder: dict) -> List[dict]:
    """
    Takes a parent folder and recursively produces a single-level list
    of all files contained in that folder and any subdirectories.
...
def get_folder_contents(folder: dict) -> List[dict]:
    ...
    :return: a list of Google drive folder/file dictionaries
    representing all contents of the folder.
    """
    ...
    if http_response.status_code == 200:
        ...
        # I figure it's better to ignore a failed folder fetch than provide no response/error out.
        return []
...
def handle_attic(command: Command) -> None:
    """
    `!attic [COURSE CODE]` - Returns a list of links to all documents in the course folder for UQ
    Attic, the unofficial exam solution and study material repository. Defaults to searching for
    the name of the current channel unless explicitly provided a course code (e.g. CSSE1001).
    """
    ...
    if len(files) > ROOM_FILE_LIMIT:
        bot.post_message(channel, 'Too many files to list here, sent the list directly to '
                                  f'<@{command.user_id}>.')
