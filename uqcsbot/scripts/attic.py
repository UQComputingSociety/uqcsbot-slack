...
def format_files(files: List[dict]) -> List[str]:
    """
    Takes the list of file dictionaries found and formats them into Slack-appropriate message
    strings.

    :param files: a list of dictionaries where each dictionary represents a file in the Google Drive
    folder.
...
def get_all_files(folder: dict) -> List[dict]:
    """
    Takes a parent folder and recursively produces a single-level list of all files contained in
    that folder and any subdirectories.
...
def get_folder_contents(folder: dict) -> List[dict]:
    ...
    :return: a list of Google drive folder/file dictionaries representing all contents of the
    folder.
    """
...
def handle_attic(command: Command) -> None:
    ...
    Attic, the unofficial exam solution and study material repository. Defaults to searching for the
    name of the current channel unless explicitly provided a course code (e.g. CSSE1001).
    """
