...
def handle_urban(command: Command) -> None:
    ...
    filtered_definitions = filter(lambda def_: def_['word'].casefold() == search_term.casefold(),
                                  results['list'])

    # Sort definitions by their number of thumbs ups.
    sorted_definitions = sorted(filtered_definitions, key=lambda def_: def_['thumbs_up'],
                                reverse=True)
    ...
    # Remove Urban Dictionary [links]
    best_definition_text = re.sub(r'[\[\]]', '', best_definition["definition"])

    # Remove Urban Dictionary [links]
    example_text = re.sub(r'[\[\]]', '', best_definition.get('example', ''))
