...
class Event(object):
    ...
    def __str__(self):
        ...
        start_str = f"{day_abbr[d1.weekday()]} {month_abbr[d1.month].upper()} {d1.day} {d1.hour}:{d1.minute:02}"
        if (d1.month, d1.day) != (d2.month, d2.day):
            end_str = f"{day_abbr[d2.weekday()]} {month_abbr[d2.month].upper()} {d2.day} {d2.hour}:{d2.minute:02}"
        else:
