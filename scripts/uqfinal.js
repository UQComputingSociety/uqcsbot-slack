	...
	robot.respond(/!?uqfinal ([a-z]{4}[0-9]{4}[a-z]?) (.+)+/i, function (res) {
        ...
        robot.http("https://api.uqfinal.com/semesters")
            .get() (function(err, resp, body) {
                ...
                semesters = JSON.parse(body).data.semesters;
                for (var i = 0; i < semesters.length; i++) {
                    res.send(JSON.stringify(semesters[i].isCurrent));
            ...
            res.send("https://api.uqfinal.com/course/" + semester + "/" + res.match[1]); 
            var uqf = robot.http("https://api.uqfinal.com/course/" + semester + "/" + res.match[1])
                    .get() (function(err, resp, body) {
                        if(err) {
                            res.send("Error: " + err);
                            return;
