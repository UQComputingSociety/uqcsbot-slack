...
def mocked_api_request(questions):
    ...
    with open('test/leetcode.json', "rb") as question_bank:
        ...
        for question in lc_data["stat_status_pairs"]:
            if (question["paid_only"] == False):
                ...
                questions[LC_DIFFICULTY_MAP[question["difficulty"]["level"] - 1]].append(lc_data)

def mocked_empty_api_call(questions):
    return questions

def mocked_easy_only(questions):
    questions['easy'].append(("Test Question", "https://www.google.com"))
