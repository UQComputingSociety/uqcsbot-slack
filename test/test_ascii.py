...
DEFAULT_FONT_RESULT = "```\n" + r'''  _______ _     _____  _____   _  _____           _______    _____ _______   __  __           _____              ______ 
...
def test_default_font(uqcsbot: MockUQCSBot):
    ...
    print(messages[1].get('text'))
    print(DEFAULT_FONT_RESULT)
...
def test_custom_font(uqcsbot: MockUQCSBot):
    ...
    messages = uqcsbot.test_messages.get(TEST_CHANNEL_ID, [])
    assert len(messages) == 2
    assert messages[1].get('text') == CUSTOM_FONT_RESULT
